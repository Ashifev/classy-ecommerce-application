...
module.exports = {
    getCart : async(req,res)=>{
        try{
            ...
            const cartProduct  = await cartDB.findOne({ userId:user.id }).populate('products.productId');
            console.log("cartProduct",cartProduct);

            if(cartProduct !== null && cartProduct.products.length > 0){
                res.render('user/userCart',{user: req.session.user,cartProduct});
            } else {
                res.render('user/userCart', {user: req.session.user,empty:"Your cart is empty"});
            }
        }catch(err){
